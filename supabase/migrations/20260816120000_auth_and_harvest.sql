-- ログイン（login_id + 暗証番号）と、収穫まわりのテーブルを追加する。
--
-- ここで入る変更は 3 つ。
--   1. 権限区分の整理と暗証番号（PIN）の保存先を workers に足す
--   2. 「どこに何がどれくらいあるか」を持つ作付情報テーブル field_plantings
--   3. 「どれくらい収穫したか」を残す harvest_records と、その集計関数
--
-- 認可の実質的な判定はアプリ側（Server Action / DAL）で行う。RLS は anon 全許可の
-- ままなので、DB 直アクセスに対する防御にはなっていない。Supabase Auth を入れる
-- ときに trial_ ポリシーごと差し替えること。
...
-- 暗証番号は scrypt でハッシュ化した文字列だけを持つ。平文は保存しない。
-- NULL は「まだ設定していない」＝初回ログイン時に本人が決める、という意味。
alter table public.workers add column if not exists pin_hash text;

comment on column public.workers.pin_hash is
  '暗証番号の scrypt ハッシュ（salt:hash 形式）。NULL は未設定で、初回ログイン時に設定させる。';
